...
crafter() {
    ...
    echo ""
    echo "showing virtual interfaces..."
    echo ""
    sleep 2
    sudo ip link show | grep mon
}
...
spooferVirt() {
    ...
    for ((i=1; i<=numINT; i++)); do
        ...
        mac="${mac_array[random]}"

        sudo ip link set dev "mon$i" down
        sudo ip link set dev "mon$i" address "$mac"
    ...
    echo    $purple"{"$cyan"1"$purple"}"$yellow"--"$green"Craft 100 virtual interfaces in monitor mode to attack multiple networks at the same time"
    echo    $purple"{"$cyan"2"$purple"}"$yellow"--"$green"Delete 100 virtual interfaces in monitor mode to attack multiple networks"
